...
if ! az acr repository show -n $acrName --image testframework/jmetermaster:latest &>/dev/null; then
    ...
    az acr build -g ${fwkrg} -t testframework/jmetermaster:latest -f ../master/Dockerfile -r $acrName .
    if [ $? -ne 0 ]
    then
...
if ! az acr repository show -n $acrName --image testframework/jmeterslave:latest &>/dev/null; then
    ...
    az acr build -g ${fwkrg} -t testframework/jmeterslave:latest -f ../slave/Dockerfile -r $acrName .
    if [ $? -ne 0 ]
    then
...
if ! az acr repository show -n $acrName --image testframework/reporter:latest &>/dev/null; then
    ...
    az acr build -g ${fwkrg} -t testframework/reporter:latest -f ../reporter/Dockerfile -r $acrName .
    if [ $? -ne 0 ]
    then
        echo "ERROR:Failed to build and push slave error: '${?}'"
        exit 1
...
echo 'INFO: AKS cluster created successfully....'
